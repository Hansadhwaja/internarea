...
function JobDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(`https://internarea.onrender.com/api/job/${id}`)
      ...
      <div className="p-3">       
          ...
          <div className="p-5 shadow-sm rounded-xl border-2">
            ...
            <div className="flex text-sm justify-between m-3">
              ...
              <p className='mt-3 text-slate-400' > <i class="bi bi-calendar-check-fill"></i>  {t("duration")} <br /> {data.Duration}</p>
